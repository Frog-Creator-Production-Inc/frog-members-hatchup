...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(`【管理者】Content Snare統合情報取得 (ID: ${requestId}, verbose: ${verbose})`);
    ...
    // まずcourse_applicationsテーブルからcontent_snare_request_idを検索
    let contentSnareRequestId;
    const { data: applicationData, error: applicationError } = await supabase
      .from('course_applications')
      .select('content_snare_request_id')
      .eq('id', requestId)
      .single();
    
    if (applicationData && applicationData.content_snare_request_id) {
      // 申請IDから取得できた場合
      contentSnareRequestId = applicationData.content_snare_request_id;
    } else {
      // 直接Content Snare Request IDが指定された場合
      contentSnareRequestId = requestId;
    }
    
    if (!contentSnareRequestId) {
      console.error("Content Snare Request IDが見つかりません");
      return NextResponse.json(
        { error: "Content Snare Request IDが見つかりません" },
        { status: 404 }
      );
    }
    ...
    console.log("リクエスト情報取得成功:", { 
      ...
      pages: responseData.pages ? responseData.pages.length : 0
    });
    ...
    console.error("Content Snare統合情報取得エラー:", error);
    return NextResponse.json({ error: "Content Snare統合情報の取得に失敗しました" }, { status: 500 });
  }
} 